...
| F-03 | Answer natural-language questions about the data. | Use Qwen to convert a question into a validated structured analytics request; execute it deterministically. | Final live orchestration run answered all five assessment questions plus five held-out questions with typed deterministic results. | PASS |
| F-04 | Use an LLM for natural-language understanding. | Call local Ollama with model `qwen2.5:3b`, temperature 0 and a schema-constrained response. | Two-stage schema-constrained client passed the 24-test live suite and final 32-case evaluation; readiness identifies the installed model and Ollama version. | PASS |
...
| DATA-09 | Evaluate Qwen on a separate labeled set. | Store expected interpretations and independently calculated answers for samples, held-out paraphrases, filters, rankings, dates, search, anomalies, ambiguity, unsupported requests and injection. | The final 32-case run passed 32/32, including 23/23 held-out cases, with zero execution failures; baseline and final reports are retained. | PASS |
...
| T-05 | Provide dependency specification. | Supply evaluator-facing `requirements.txt` with tested, pinned versions. | A new Python 3.11 environment installed all exact direct pins; the latest working-tree run passes 198 normal tests, the clean run passed its complete 196-test suite, all 24 live tests pass, and `pip check` passes. | PASS |
...
| E-03 | Code quality — 20% | Modular code, meaningful names, validation, error handling, automated tests and clean checks. | Ruff, compilation, 198 deterministic/integration tests, 24 live tests, dependency checks, privacy-safe diagnostics, repository publication audit and process cleanup all pass. | PASS |
| E-04 | LLM integration quality — 15% | Structured prompt and output, validation, retry policy, ambiguity handling, injection resistance and live-model evaluation. | Two-stage routing/extraction, deterministic constraint preservation, one corrective retry, typed failures, 24 live tests and the final 32/32 labeled evaluation are documented and passing. | PASS |
